...
namespace AlvinSoft.TcpMs.Packages {
    ...
    /// <summary>Represents the settings of a <see cref="TcpMsServer"/>.</summary>
    public class ServerSettings {
        ...
        /// };
        /// </code></value>
        public static ServerSettings None => new ServerSettings(null) {
            Version = -1
        };
...
#pragma warning restore CS1591
        ...
        public readonly DataTypes DataType { get; }

        /// <summary>The purpose of the package</summary>
        public readonly PackageTypes PackageType { get; }
        ...
        internal readonly TaskCompletionSource<byte>? TaskCompletion;
        ...
        internal Package(PackageTypes type, bool useTask = false) {
            ...
            TaskCompletion = useTask ? new TaskCompletionSource<byte>() : null;
        ...
        internal Package(PackageTypes packageType, DataTypes dataType, byte? data, bool copyData = true, bool useTask = false) : this(packageType, dataType, new byte[] { data ?? 0 }, copyData, useTask) { }
        ...
        public async Task Await(CancellationToken cancellationToken = default) {
            ...
            using (cancellationToken.Register(() => taskCompletion.TrySetCanceled())) {
                await TaskCompletion.Task;
            }
        ...
        public override int GetHashCode() => HashCode.Combine(DataType, PackageType, Data, IsInternalPackage, IsEmpty);
        #endregion
...
#nullable restore
    ...
    internal abstract class PackageHandler {
        ...
        public PackageHandler(NetworkStream targetStream) {
            Stream = targetStream;
        }
        ...
        private async Task ObtainHandler() {
            ...
            ObtainHandlerCancel = new CancellationTokenSource();
            ObtainHandlerPause = new ManualResetEventSlim(true);
            ObtainHandlerSync = new SemaphoreSlimUsing();
        ...
        private async Task DispatchHandler() {
            ...
            QueueOut = new ConcurrentQueue<Package>();
            DispatchHandlerCancel = new CancellationTokenSource();
            DispatchHandlerPause = new ManualResetEventSlim(true);
            DispatchHandlerSync = new SemaphoreSlimUsing();
        ...
        /// </summary>
        /// <remarks>This task runs synchronously like <see cref="Send(Package)"/> if <paramref name="package"/> does not have a <see cref="TaskCompletionSource{TResult}"/> assigned.</remarks>
        /// <exception cref="ObjectDisposedException"/>
        ...
        public async Task<Package> ObtainPackageAsync(CancellationToken cancellationToken = default) {
            ...
            try {
                ...
                return new Package(packageType, dataType, dataBuffer, false);
    ...
    /// </summary>
    [Serializable]
    internal class TcpMsUnexpectedPackageException : Exception {
        /// <summary>
        /// Thrown when a TcpMs instance receives an unexpected package.
        /// </summary>
        public TcpMsUnexpectedPackageException(string expected, string receivedPackage) : base($"Received package of type \"{receivedPackage}\", but expected \"{expected}\"") { }
    ...
    /// </summary>
    [Serializable]
    internal class TcpMsErrorPackageException : Exception {

        /// <summary>
        /// Thrown when a TcpMs instance receives an error package.
        /// </summary>
        public TcpMsErrorPackageException() : base("Received an error package.") { }
    ...
    /// </summary>
    [Serializable]
    internal class TcpMsTimeoutException : Exception {
        public TcpMsTimeoutException() : base("Timed out while obtaining a package.") { }
    }
}